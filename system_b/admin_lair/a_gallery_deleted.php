...
CMain::getLangFile();
global $USER;
if($USER->hr('read_deleted_gallery')):
...
if(isset($_REQUEST['g_target'])&&isset($_REQUEST['action']))
{
	...
	elseif($_REQUEST['action']=='restore'&&count($_REQUEST['g_target'])>0)
	{
		if(!$USER->hr('restore_image_wink'))
		{
			CAdminPage::errorMsg(CMain::getM('ERROR_CANT_RESTORE_IMAGE'));
		}
