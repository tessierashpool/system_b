...
class CVGallery
{
	private static $table_name='t_video_gallery_list';
	private static $t_group_rights='t_vgallery_rights';
	...
	{	
		...
		$id = intval($id);
		$sql = "DELETE FROM `".CVideos::getTableName()."` WHERE `gallery_id`='$id'";
		$result = CDateBase::deleteQuery($sql);
		return $result;
	}		
	...
	public static function size($id)	
	{
		...
		CMain::includeClass('videos.CVideos');
		$id = intval($id);
		$sql = "SELECT COUNT(*) FROM `".CVideos::getTableName()."` WHERE `gallery_id`='$id'";	
		$result = CDateBase::selectQuery($sql);	
