...
class CSettings
{
	...
	{
		$sql = "SELECT * FROM `t_site_settings`";
	...
	public static function add($name,$value)
	{
		...
		$sql = "INSERT IGNORE INTO `t_site_settings` SET `name` = '".$name."', `value` = '".$value."'";
		$res = mysql_query($sql) or die("Invalid query: " . mysql_error());	
		return $res;
	}
	...
	public static function update($settings = array())
	{
		foreach($settings as $name => $value)
		{
			$name = safeStr($name);
			$value = safeStr($value);		
			$sql = "UPDATE `t_site_settings` SET `value` = '".$value."' WHERE `name` = '".$name."' LIMIT 1";
			$res = mysql_query($sql) or die("Invalid query: " . mysql_error());	
		}
