...
function leftColumnHeight()
{
	...
	var body_min_height = c_height-header_h-footer_h;
	$('.body_main_cont').css('min-height',body_min_height+'px');
	$('.body_cont').css('min-height',body_min_height+'px');
	var margin_t = parseInt($('.left_column').css('margin-top'));
	var margin_b = parseInt($('.left_column').css('margin-bottom'));
	var left_column_min_height = $('.body_cont').height()-margin_t-margin_b;
	$('.left_column').css('min-height',left_column_min_height+'px');
}
function imgDescrWidth(){
	$('.img_description').each(function(){
		var i_width = $(this).prev('.image_cont').children('img').width();
		$(this).width(i_width);
	});
}
...
function newPostLoadVideo(){
	...
	if(parse_url = youtube_parser(newVidUrl))
	{
		...
		vidObjText = '<div class="posted_video_cont"><object width="600" height="337">'
...
//Hide and show base vertical scrollbar
function scrollShowHide(hide)
{
	if(document.documentElement.scrollHeight>document.body.clientHeight)
	{
		if(hide=="h")
