...
class CUsersMain{
	...
	private static $table_name = 't_users_additional';
	...
	public static function registNewUser($login,$password, $email='', $active_confirmation=1, $active='Y')
	{
		...
		$sql = "INSERT INTO `".self::$table_name."` (`id`, `login`, `pass`, `email`, `active_confirmation`, `date_create`, `date_update`, `date_update_timestamp`, `date_create_timestamp`, `active`) VALUES (NULL, '".$login."', '".$u_password."', '".$email."', '".$active_confirmation."', ".time().", ".time().", NOW(), NOW(),'".$active."')";
		$result = mysql_query($sql);	
		self::$last_id =  mysql_insert_id();
		return $result;
	}
	
	public static function update($login,$password, $email='', $active='Y', $id)	
	{
		CMain::includeClass('db.CDateBase');
		$arFields['login'] = $login;
		$arFields['pass'] = self::passwordHash($password);
	...
	public static function resetPassword($id, $hash, $password)
	{
		...
		//	self::clearPassRestore($myrow['user_id']);
			$id = $myrow['user_id'];
			$u_password = self::passwordHash($password);
			$sql = "UPDATE `".self::$table_name."` SET `pass` = '$u_password', `date_update` = ".time().", `date_update_timestamp` = NOW() WHERE `id` = $id LIMIT 1";
			$result = mysql_query($sql);	
			return $result;
		}
		else
